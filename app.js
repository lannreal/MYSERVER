...
async function runSolver() {
  ...
  try {
    ...
    // Proxy dihapus: menggunakan IP asli mesin
    const dynamicArgs = [...CHROME_ARGS];
...
let currentSession = null;
    ...
    while (Date.now() < deadline) {
      ...
      let challenging = await isChallenging(page);
      ...
      if (cfCookie) {
        ...
        let isClean = await isCleanDashboard(page);
        if (page._forceBypass) { challenging = false; isClean = true; }
        
        if (challenging || !isClean) {
